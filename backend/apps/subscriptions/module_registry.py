"""Canonical school-admin module bundles (15 core modules)."""
from __future__ import annotations

from typing import Any

# Each module: primary nav entry + child features from the plan catalog.
SCHOOL_MODULES: list[dict[str, Any]] = [
    {
        ...
        "sort_order": 0,
        # School Settings, Roles & Permissions, and Delete User are platform core
        # capabilities (not sellable plan modules). They must not appear in the
        # super-admin plan editor catalog or school module hub.
        "feature_keys": [
    ...
    },
    {
        ...
        "children": [
            ...
            {"feature_key": "class_report_cards", "label": "Class Report Cards", "path": "/school-admin/examinations/class-report-cards", "icon": "FiFileText"},
...
def get_module_for_feature(feature_key: str) -> dict[str, Any] | None:
    for module in SCHOOL_MODULES:
        if feature_key in module["feature_keys"]:
            return module
    return None


def all_module_feature_keys() -> set[str]:
    keys: set[str] = set()
    for module in SCHOOL_MODULES:
        keys.update(module["feature_keys"])
    return keys